// ZIP code prefix to state mapping based on USPS ZIP code ranges.
// This maps the first 3 digits of a ZIP code to the correct state(s).
// For prefixes that span multiple states, we use the most common state.
...
/**
 * Look up the US state abbreviation for a given 5-digit ZIP code.
 * Returns undefined if the ZIP prefix is not recognized.
 */
export function getStateFromZip(zip: string): string | undefined {
  if (!/^\d{5}$/.test(zip)) return undefined;
  return ZIP_PREFIX_TO_STATE[zip.substring(0, 3)];
}

/**
 * Get the 3-digit ZIP prefix for proximity filtering.
 * Hospitals sharing a ZIP prefix are typically within ~50 miles.
 */
export function getZipPrefix(zip: string): string {
